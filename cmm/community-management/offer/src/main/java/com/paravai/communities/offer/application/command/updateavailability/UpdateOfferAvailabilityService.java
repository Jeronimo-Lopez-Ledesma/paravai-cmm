...
@Service
public class UpdateOfferAvailabilityService {

    private static final Logger log = LoggerFactory.getLogger(UpdateOfferAvailabilityService.class);
    ...
    public UpdateOfferAvailabilityService(
     ...
     * - persistence
     * - OfferAvailabilityUpdated event emission when there is an effective change
     *
     * NOT covered here:
     * - external cross-module validations
     */
    public Mono<UpdateOfferAvailabilityResult> updateAvailability(
            IdValue offerId,
        ...
        Objects.requireNonNull(availabilityStatusCode, "availabilityStatusCode is required");
        ...
        return Mono.deferContextual(ctx -> {
            ...
            log.info("[update-offer-availability-service] Reactor context - traceId={}, userOid={}, sourceSystem={}",
                    traceId, userOid, sourceSystem);

            final OperationCtx opCtx = MetricsSupport.withSourceSystem(
                    OfferMetrics.ID.app("updateOfferAvailability"),
                    sourceSystem
            );

            return MetricsSupport.timedMono(metrics, opCtx, () -> {
                final OfferAvailabilityStatusValue newAvailabilityStatus =
                ...
                return offerRepository.findById(offerId)
                        ...
                        .flatMap(existing -> {
                            ...
                            boolean changed = existing.updateAvailability(
                                    currentUserId,
                            ...
                            if (!changed) {
                                return Mono.just(UpdateOfferAvailabilityResult.unchanged(existing));
                            }

                            return offerRepository.save(existing)
                                    .flatMap(saved -> publishOfferAvailabilityUpdatedEvent(
                                            saved,
                                            previous,
                                            traceId,
                                            userOid,
                                            sourceSystem
                                    ).thenReturn(UpdateOfferAvailabilityResult.updated(saved)));
                        })
                        .doOnError(ex -> {
                            if (ex instanceof CustomException) {
                                log.warn(
                                        "[{}][{}] Business error while updating availability for offer {}: {}",
                                        traceId,
                                        userOid,
                                        offerId,
                                        ex.getMessage()
                                ...
                                log.error(
                                        "[{}][{}] Failed to update availability for offer {}",
                                        traceId,
                                        userOid,
                                        offerId,
                                        ex
    ...
    private Mono<Void> publishOfferAvailabilityUpdatedEvent(
            Offer saved,
            JsonNode previous,
            String traceId,
            String userOid,
        ...
        EntityChangedEvent evt = eventFactory.build(
                ...
                userOid,
                sourceSystem,
                "Offer availability updated: " + saved.id(),
                previous,
                current
        );
