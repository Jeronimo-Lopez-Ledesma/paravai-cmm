...
import com.paravai.communities.offer.application.command.create.exception.DuplicateActiveOfferException;
    ...
    public class CreateOfferService {
        ...
        public Mono<CreateOfferResult> create(
            ...
            return Mono.deferContextual(ctx -> {
                ...
                log.info("[create-offer-service] Reactor context - traceId={}, userOid={}, sourceSystem={}",
                ...
                return MetricsSupport.timedMono(metrics, opCtx, () -> {
                    ...
                    return offerRepository.findActiveByTenantIdAndCommunityIdAndResourceId(
                            ...
                            .flatMap(existing -> Mono.<CreateOfferResult>error(
                                    new DuplicateActiveOfferException(
                                            resourceId.value(),
                                            communityId.value()
                                    )
                            ))
                            ...
                            .doOnError(ex -> {
                                if (ex instanceof com.paravai.foundation.domain.exception.CustomException) {
                                    log.warn(
                                            "[{}][{}] Business error while creating offer for resource {} in community {}: {}",
                                            traceId,
                                            userOid,
                                            resourceId,
                                            communityId,
